...
class WinEventReader {
    ...
    _processLogEvent(event) {
        ...
        // NOTE(cristeigabriel): Apparently, the ProcessID seems to be
        // always wrong (always 4 after ConvertTo-JSON even though
        // `@{Booba=0x1234} | ConvertTo-Json` outputs the right value.)
        // Therefore, we're going to (oh fucking hell) manually parse
        // the numbers we care about from the message.
        function readProcessId(from, new_or) {
            // Determine query based on input.
            let query = new_or ? '\tNew Process ID:\t' : '\tProcess ID:\t'
            let delimiter = '\r\n'

            // Get query entry index.
            let entryIndex = from.indexOf(query)
            if (entryIndex === -1) {
                return null
            }

            // Get remainder of string from query value.
            let remainder = from.slice(entryIndex + query.length, from.length)

            // Retrieve integer from hex string.
            let valueEndIndex = remainder.indexOf(delimiter)
            let value = parseInt(remainder.slice(0, valueEndIndex), 16)
            if (value === NaN) {
                return null
            }
        ...
        return {
            ...
            processId: readProcessId(event.Message, false),
            newProcessId: readProcessId(event.Message, true),
